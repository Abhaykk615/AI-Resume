...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    // Support different env var names (fallback to values in .env)
    const jwtSecret = process.env.JWT_SECRET_KEY || process.env.JWT_SECRET;
    const jwtExpire = process.env.JWT_SECRET_EXPIRES_IN || process.env.JWT_EXPIRE;
    ...
    const cookieOptions = {
      ...
      // In development allow lax so cookies work on localhost; in production use None with secure
